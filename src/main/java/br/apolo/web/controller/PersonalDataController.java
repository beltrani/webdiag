...
public class PersonalDataController extends BaseController<PersonalData> {
	...
	public ModelAndView list(HttpServletRequest request) {
		breadCrumbService.addNode(MessageBundle.getMessageBundle("breadcrumb.personalData.list"), 1, request);
		
		ModelAndView mav = new ModelAndView(Navigation.PERSONALDATA_LIST.getPath());
		
		List<PersonalData> personalDataList = personalDataService.list();
		
		mav.addObject("personalDataList", personalDataList);
	...
	public ModelAndView save(@Valid @ModelAttribute("entity") PersonalData entity, BindingResult result,
		...
		if (result.hasErrors()) {
			...
			for (ObjectError error : result.getAllErrors()) {
				...
				message.append(MessageBundle.getMessageBundle("common.field") + " " + MessageBundle.getMessageBundle("personalData." + argument.getDefaultMessage()) + ": " + error.getDefaultMessage() + "\n <br />");
			}
	...
	public ModelAndView view(@PathVariable Long id, HttpServletRequest request) {
		breadCrumbService.addNode(MessageBundle.getMessageBundle("breadcrumb.personalData"), 2, request);
		
		ModelAndView mav = new ModelAndView(Navigation.DOCTOR_VIEW.getPath());
		
		PersonalData personalData = personalDataService.find(id);
	...
	public ModelAndView create(HttpServletRequest request) {
		breadCrumbService.addNode(MessageBundle.getMessageBundle("breadcrumb.personalData.new"), 1, request);
		
		ModelAndView mav = new ModelAndView(Navigation.PERSONALDATA_NEW.getPath());
		...
		personalData.setCreatedBy(personalDataService.getAuthenticatedUser());
		personalData.setCreationDate(new Date());
		
		personalData.setLastUpdatedBy(personalDataService.getAuthenticatedUser());
		personalData.setLastUpdateDate(new Date());
	...
	public ModelAndView edit(@PathVariable Long id, HttpServletRequest request) {
		breadCrumbService.addNode(MessageBundle.getMessageBundle("breadcrumb.personalData.edit"), 2, request);
		
		ModelAndView mav = new ModelAndView(Navigation.PERSONALDATA_EDIT.getPath());
	...
	public @ResponseBody String remove(@PathVariable Long id) {
		...
		if (personalData != null) {
			try {
				personalDataService.remove(personalData);
